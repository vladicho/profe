...
const VIDEO_PLAY={impact:[1.05,.41,.06] as V3,bounce1:[-.8,.05,7.2] as V3,bounce2:[1.45,.05,12.3] as V3};
const LABELS={
...
export default function Fronton3D({lang}:{lang:Lang}){
  ...
  return <div className="fronton3d" ref={container}>
    ...
    <div className="threeStage"><canvas ref={canvas} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={()=>{drag.current=null}} aria-label={copy.title}/><div className="threeBadge"><span>WEBGL</span><b>GLB</b><span>POSE MOTION</span></div></div>
...
function updateCamera(runtime:Runtime,yaw:number,scenario:Scenario,time:number){const zoom=scenario==="video"?videoZoom(time):0,target=new THREE.Vector3(0,lerp(2.25,.43,zoom),lerp(8,.12,zoom)),radius=lerp(24,2.35,zoom),pitch=.34,angle=lerp(yaw,.18,zoom);runtime.camera.position.set(target.x+Math.sin(angle)*Math.cos(pitch)*radius,target.y+Math.sin(pitch)*radius,target.z+Math.cos(angle)*Math.cos(pitch)*radius);runtime.camera.lookAt(target)}
...
function videoBallPosition(time:number):V3{const t=clamp(time,0,1);if(t<=.28)return arc(poseRacketGeometry(0,0).head,VIDEO_PLAY.impact,t/.28,1.55);if(t<=.62)return arc(VIDEO_PLAY.impact,VIDEO_PLAY.bounce1,(t-.28)/.34,1.7);if(t<=.84)return arc(VIDEO_PLAY.bounce1,VIDEO_PLAY.bounce2,(t-.62)/.22,.82);return arc(VIDEO_PLAY.bounce2,[2.05,.05,13.4],(t-.84)/.16,.28)}
