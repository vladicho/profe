...
type Lang = "es" | "pt";
type Scenario = "lata" | "special2" | "special3" | "suncho";
type V3 = [number, number, number];

const SCENARIOS: Record<Scenario,{strike:V3;impact:V3;score:"against"|"1"|"2"|"3";metal:boolean}> = {
...
const LABELS = {
  es:{title:"Cancha 3D interactiva",help:"Arrastra para girar la cámara. El valor especial depende de dónde el jugador golpea.",lata:"Lata",two:"Especial · 2",three:"Especial · 3",suncho:"Suncho",play:"Reproducir",replay:"Repetir",against:"PUNTO EN CONTRA",points:"PUNTOS",impact:"CONTACTO",strike:"GOLPE DEL JUGADOR",near:"ZONA 2",far:"ZONA 3",frontis:"FRONTIS",metal:"SONIDO METÁLICO"},
  pt:{title:"Quadra 3D interativa",help:"Arraste para girar a câmera. O valor especial depende de onde o jogador rebate.",lata:"Lata",two:"Especial · 2",three:"Especial · 3",suncho:"Suncho",play:"Reproduzir",replay:"Repetir",against:"PONTO CONTRA",points:"PONTOS",impact:"CONTATO",strike:"REBATIDA DO JOGADOR",near:"ZONA 2",far:"ZONA 3",frontis:"FRONTIS",metal:"SOM METÁLICO"},
};
...
export default function Fronton3D({lang}:{lang:Lang}){
  ...
  const [scenario,setScenario]=useState<Scenario>("special3");
  const [run,setRun]=useState(0);
  const [yaw,setYaw]=useState(.56);
  const [playing,setPlaying]=useState(false);
  const [status,setStatus]=useState("");
  const copy=LABELS[lang],shot=SCENARIOS[scenario];
  ...
  useEffect(()=>{
    ...
    const paint=(now:number)=>{
      ...
      const elapsed=playing?Math.min(1,(now-start)/4200):(run?1:0);
      drawScene(x,width,height,yaw,shot,elapsed,copy);
      if(playing&&elapsed>=.43&&!sounded){sounded=true;setStatus(shot.score==="against"?copy.against:`${shot.score} ${copy.points}`);if(shot.metal)metalSound(audio.current)}
  ...
  function start(){
    ...
    audio.current.resume().catch(()=>{});setStatus("");setRun(value=>value+1);setPlaying(true);
  }
  ...
  return <div className="fronton3d">
    <div className="fronton3dHead"><div><strong>{copy.title}</strong><small>{copy.help}</small></div><output className={shot.score==="against"?"against":""}>{status||"3D"}</output></div>
...
function drawScene(x:CanvasRenderingContext2D,width:number,height:number,yaw:number,shot:(typeof SCENARIOS)[Scenario],time:number,copy:(typeof LABELS)[Lang]){
